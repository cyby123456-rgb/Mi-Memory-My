...
| App. F Alg. 2--3, Table 28, Eq. 19 | E2MEND Observe/Improve/Verify, five gates, Critic, UCB1, pending champion, drift rollback | `evolution.py`, `strategy.py` | Implemented: gate artifacts, history, UCB1 scoring, rollback threshold; pending-champion selection and prompt-version mutations pending |
| Sec. 7; App. G Eq. 20--21, Alg. 4 | File-native router, lazy daily reads, style/profile priority, decay, idle consolidation, atomic index, Git | `storage.py`, `lifecycle.py`, `git_provenance.py` | Implemented: singleton style/profile, correction file, line-window router, priority injection, markers, atomic index; exact formula calibration pending |
